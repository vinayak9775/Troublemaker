...
import { Grid, Box, Button, Snackbar, AppBar, Alert, CircularProgress, Checkbox, Tooltip, FormControl, FormLabel, FormControlLabel, Radio, RadioGroup, Card, CardContent, Stack, Typography, TextField, Menu, Modal, MenuItem, Table, TableHead, TableRow, TableBody, TableContainer, TablePagination } from '@mui/material';
...
const Viewservice = () => {
    ...
    const { eventID, patientID, callerID, eventPlanID, flag } = location.state;
    console.log(flag, 'fffffffffffffff');
    ...
    useEffect(() => {
        const getProfessional = async () => {
            if (ptnZoneID) {
                setLoading(true);
                try {
                    let apiUrl = `${port}/web/agg_hhc_event_professional_api/?eve_poc_id=${eventPlanID}&zone=${ptnZoneID}`;
                    ...
                    if (zoneID) {
                        ...
                        apiUrl = `${port}/web/agg_hhc_event_professional_api/?srv=${serviceID}&prof_name=${searchProf}`;
                        console.log("apiUrl", apiUrl)
                    }
        ...
        getProfessional();
    }, [ptnZoneID, eventPlanID, zoneID, serviceID, searchProf]);
    ...
    async function handleMultiAllocation(event) {
        ...
        // if (flag === '2') {
        //     requestData.remark = remark; // Replace with the actual remark value
        // }
                        ...
                        <Grid item xs={12} sm={12} md={12} lg={6}>
                            <Card style={{ background: '#ffffff', boxShadow: '4px 4px 10px 7px rgba(135, 135, 135, 0.05)', borderRadius: '10px', }}>
                                <CardContent>
                                    ...
                                    <Box sx={{ height: "auto", marginTop: "5px" }}>
                                        <TableContainer sx={{ height: professional.length === 0 || professional.length < 5 ? "48vh" : "default" }}>
                                            <Table>
                                                <TableHead >
                                                    <TableRow>
                                                                    ...
                                                                    >
                                                                        ...
                                                                        >
                                                                                    ...
                                                                                    <Box sx={{ ...style, width: 350, borderRadius: "5px" }}>
                                                                                        <Grid container spacing={2} sx={{ mt: 0.5 }}>
                                                                                            ...
                                                                                            <Grid item lg={12} sm={12} xs={12}>
                                                                                             ...
                                                                                             {flag==1 && <Grid item lg={12} sm={12} xs={12} sx={{ mt: 1 }}>
                                                                                                <FormControl>
                                                                                                    <div style={{ display: "flex" }}>
                                                                            ...
                                                                            <CardContent style={{ flex: 1.5 }}>
                                                                                <Typography variant='body2'>{row.work_phone_no}</Typography>
                                                                            </CardContent>
                                                                            <CardContent style={{ flex: 2 }}>
                                                                                <Typography variant='body2' textAlign="center">{row.prof_zone_id}</Typography>
                                                                            </CardContent>
