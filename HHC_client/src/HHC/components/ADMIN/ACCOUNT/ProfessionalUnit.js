...
const ProfessionalUnit = () => {
    ...
    const downloadExcelSheetprofID = async (prof_id) => {
        try {
            const url = `${port}/hhc_account/View_professional_unit_report22/${prof_id}/${startDate}/${endDate}/`;
            ...
            // Assuming the API response includes the CSV content directly
            const text = await res.text(); // Get CSV data as text

            // Example: Extract professional name from the API response
            const lines = text.split('\n'); // Split CSV into lines
            let professionalName = ''; // Initialize variable to hold professional name
            for (let line of lines) {
                if (line.startsWith('Details Of Professional:')) {
                    professionalName = line.substring('Details Of Professional:'.length).trim();
                    break;
                }
            }
            ...
            // Example filename format: ProfName_Report_StartDate_EndDate.csv
            const filename = `${professionalName.replace(' ', '_')}_Report_${startDate}_${endDate}.csv`;
            ...
            window.URL.revokeObjectURL(blobUrl); // Clean up: revoke the blob URL
    ...
    // const downloadExcelSheetprofID = async (prof_id) => {
    //     try {
    //         // Fetch professional data if not available already
    //         if (!profData || !profData.details_prof || !profData.prof_cost_dtl) {
    //             const response = await fetch(`${port}/hhc_account/View_professional_unit_report/${prof_id}/${startDate}/${endDate}/`);
    //             if (!response.ok) {
    //                 throw new Error(`Failed to fetch professional with prof_id ${prof_id}`);
    //             }
    //             const fetchedData = await response.json();
    //             setProfData(fetchedData); // Update profData state with fetched data
    //         }

    //         // Check if profData is now available
    //         if (!profData || !profData.details_prof || !profData.prof_cost_dtl) {
    ...
    //         const profName = profData.Prof_name || 'Unknown';
            ...
            <div className="table-responsive" id="hospitalData" style={{ display: "none" }}>
                <table border="1" className="table table-hover table-bordered" style={{ marginBottom: '30px' }}>
                    <thead>
                        ...
                        <tr style={{ textAlign: 'center' }}>
                            <th colSpan="9" id="professionalName" style={{ whiteSpace: 'pre-wrap', fontWeight: 'bold' }}>Details Of Professional: {hospDataTable.length > 0 ? hospDataTable[0].Prof_name : ''}</th>
                        </tr>
                        <tr>
                            <th>Voucher Date</th>
                            <th>Party Name</th>
