...
const JobClosureAccount = () => {
    ...
    const handleViewReport = async () => {
        ...
        try {
            let url = `${port}/hhc_account/job_closure_report_prof_wise/`;
    ...
    useEffect(() => {
        const handleViewResponse = async () => {
            try {
                setLoading(true);
                let url = `${port}/hhc_account/job_closure_report_prof_id/`;
                if (selectedID) {
                    url += `${selectedID}/`;
                }
                if (startDate) {
    ...
    const handleDownloadExcel = async () => {
        try {
            ...
            const renamedData = data.map(item => ({
                ...
                'Mobile Number': item.prof_number,
                'Total Session': item.total_session,
                // 'Mobile Number': item.mobile_no,
                'Closed Session': item.closed_session,
                'Pending Session': item.Pending_session,
    ...
    const handleModalOpen = (jobClosure) => {
        ...
        setSelectedID(jobClosure.prof_id); // Set selectedID here
                                            ...
                                            .map((jobClosure, index) => (
                                                <TableRow key={index}>
                                                    <PaymentPatientCard style={{ height: '3rem', background: "white", color: "rgba(0, 0, 0, 0.87)", fontWeight: "100", borderRadius: "8px 10px 8px 8px" }}>
                                                        ...
                                                        <CardContent style={{ flex: 3, borderRight: "1px solid #FFFFFF" }}>
                                                            <Typography variant='subtitle2'>{jobClosure.Prof_name || '-'}</Typography>
                                                        </CardContent>
                                                        <CardContent style={{ flex: 3, borderRight: "1px solid #FFFFFF" }}>
                                                            <Typography variant='subtitle2'>{jobClosure.prof_number || '-'}</Typography>
                                                        </CardContent>
                                                        {/* <CardContent style={{ flex: 3, borderRight: "1px solid #FFFFFF" }}>
                                        ...
                                        {viewTableResponse.map((jobClosure, index) => (
                                            <TableRow key={index}>
                                                <PaymentPatientCard style={{ height: 'auto', background: "white", color: "rgba(0, 0, 0, 0.87)", fontWeight: "100", borderRadius: "8px 10px 8px 8px" }}>
                                                    ...
                                                    <CardContent style={{ flex: 3, borderRight: "1px solid #FFFFFF" }}>
                                                        <Typography variant='subtitle2'>{jobClosure.service_name || '-'}</Typography>
                                                    </CardContent>
                                                    <CardContent style={{ flex: 3, borderRight: "1px solid #FFFFFF" }}>
                                                        <Typography variant='subtitle2'>{jobClosure.sessions || '-'}</Typography>
                                                    </CardContent>
                                                    <CardContent style={{ flex: 3, borderRight: "1px solid #FFFFFF" }}>
