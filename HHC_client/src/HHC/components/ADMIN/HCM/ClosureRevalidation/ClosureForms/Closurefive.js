...
const Closurefive = ({ getSessionDetils, formNo, evePlanID, profSD, profST, profED, profET, sessions, onClose, eveID }) => {
    const port = process.env.REACT_APP_API_KEY;
    const accessToken = localStorage.getItem('token');
    console.log("Closure Form 5", eveID)
    ...
    const [catheter, setCatheter] = useState(null);
    const [cathSize, setCathSize] = useState(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setCatheter(sessionDetails[0]?.Catheter_type || "");
                setCathSize(sessionDetails[0]?.Size_catheter || "");
                setProcedure(sessionDetails[0]?.Procedure || "");
                setRemark(sessionDetails[0]?.Remark || "");
                setClosureRevalidateRemark(sessionDetails[0]?.closure_revalidate_remark || "");
                console.log("Closure Form 5", sessionDetails)
    ...
    return (
        <>
            <Box sx={{ flexGrow: 1, bgcolor: "#ffffff", overflowY: "auto", p: 2, maxHeight: "80vh" }} >
                ...
                <Grid item xs={12} container spacing={1} sx={{ mt: 2 }}>
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={12}>
                        ...
                        <TextField id="standard-basic" label="Remark" variant="outlined" multiline
                            rows={2} sx={{ width: "100%" }} name="Remark"
