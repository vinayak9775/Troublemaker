...
from hhcweb.models import agg_com_colleague, agg_hhc_professional_zone, agg_hhc_service_professionals, agg_hhc_detailed_event_plan_of_care, agg_mas_group, sos_details
...
class agg_hhc_session_job_closure_serializer_form_1(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['srv_prof_id','dtl_eve_id', 'prof_sub_srv_id','Baseline','Airway', 'Breathing', 'Circulation', 'Temp_core','TBSL','Pulse','SpO2','RR','GCS_Total','BP','Skin_Perfusion','Remark', 'last_modified_by']
...
class agg_hhc_session_job_closure_serializer_form_6(serializers.ModelSerializer):
    class Meta:
        model = models.agg_hhc_jobclosure_detail
        fields = ['srv_prof_id','dtl_eve_id', 'prof_sub_srv_id', 'Remark', 'last_modified_by']
...
#     class Meta:
#         model = models.agg_hhc_payment_details
#         fields = ['pay_dt_id','eve_id', 'Total_cost', 'paid_by', 'amount_paid', 'amount_remaining', 'date','mode', 'pay_recived_by','cheque_number','cheque_date',
#                   'cheque_image',
#                   'Remark','online_payment_by','srv_prof_id', 'last_modified_by','overall_status']
...
#     #     clg_data = agg_com_colleague.objects.filter(clg_ref_id = instance.added_by)
#     #     # print(clg_data, ';;;ddddddd')
#     #     return (clg_data[0].clg_first_name+' '+clg_data[0].clg_mid_name+' '+clg_data[0].clg_last_name)
...
class agg_hhc_service_enquiry_list_serializer(serializers.ModelSerializer):
    ...
    # class Meta:
    #     model=models.agg_hhc_events
    #     fields = ('eve_id','event_code', 'patient_service_status', 'agg_sp_pt_id','srv_id', 'folloup_id','Suffered_from', 'follow_up','caller_no')     #amit
...
class create_plane_of_care_serializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['eve_poc_id','eve_id','srv_id','start_date','start_time','hosp_id', 'doct_cons_id', 'last_modified_by']
...
class add_service_get_POC_serializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['eve_poc_id','srv_id','sub_srv_id','start_date','end_date','prof_prefered','remark','start_time', 'end_time','No_session_dates' ] 
    ...
    def get_job_closure(self, obj):
        ...
        if detail_eve:
            ...
            if detail_eve[0].actual_StartDate_Time <today:
                ...
                # prof_data=detail_eve.filter(actual_StartDate_Time=today).srv_prof_id.last()
                prof_data=detail_eve.filter(actual_StartDate_Time=today).last()
                prof_data = prof_data.srv_prof_id
                prof=prof_data.prof_fullname
                prof_id=prof_data.srv_prof_id
            detail_eve
            # print(today)
...
class Add_inquiry(serializers.ModelSerializer):
    class Meta:
        model = models.Spero_Inquiry
        fields = '__all__'
...
class FeedBackQuestionSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ('F_questions', 'Questions', 'Question_hin', 'Question_mar')
