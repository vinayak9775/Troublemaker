...
def get_tokens_for_user(user):
    ...
    return {
        ...
        "colleague": {
                ...
                'prof_compny': user.prof_compny.company_pk_id,
...
class agg_hhc_sub_services_api(APIView):
    ...
    def get(self,request,pk,format=None):
        ...
        if (datetime.today().date())<=(datetime(2025, 1, 1).date()):
            ...
            sub_service= agg_hhc_sub_services.objects.filter(srv_id=pk,status=1,Entry__in=[2,3]).order_by('recommomded_service')
            serializer= agg_hhc_sub_services_serializer(sub_service,many=True)
            return Response(serializer.data)
...
class Caller_details_api(APIView):
    ...
    def put(self,request,pk):
        ...
        try:
            # Check if the phone number already exists (excluding the current caller)
            existing_caller = agg_hhc_callers.objects.filter(
                phone=request.data.get('phone')
            ).exclude(caller_id=pk).first()

            if existing_caller:
                return Response(
                    # {"error": f"Phone number already exists with Caller ID:{existing_caller.caller_id},Caller Name:{existing_caller.caller_fullname},Caller Number:{existing_caller.phone}."},
                    # status=status.HTTP_409_CONFLICT,
                    {
                        
                        "Message": f"Phone number already exists",
                        "Caller_id":existing_caller.caller_id,
                        "Caller_Name": existing_caller.caller_fullname,
                        "Caller_Number": existing_caller.phone,
                    },
                    status=status.HTTP_409_CONFLICT
                )
                

            # Retrieve the caller object
            caller = self.get_object(pk)
            ...
            if callerSerializer.is_valid():
                callerSerializer.validated_data['last_modified_date'] = timezone.now()
                callerSerializer.save()
                return Response(callerSerializer.data, status=status.HTTP_200_OK)
            return Response(callerSerializer.errors, status=status.HTTP_400_BAD_REQUEST)
        except agg_hhc_callers.DoesNotExist:
...
# --------------------------------------------------------------------------------------
        ...
        if event.data:
            ...
            data={
                "eve_id" : payment_serializer.data[-1]['eve_id'], 
                "total_cost":int(event1.Total_cost/sessions.count()) if sessions.count()!=0 else 0, 
                "Total_Amount" : float(payment_serializer.data[-1]['Total_cost']), 
                "Paid_Amount" : paid_amt, 
                # "Pending_Amount" : float(payment_serializer.data[-1]['Total_cost']) - paid_amt,
                "Pending_Amount" : event1.final_amount - paid_amt,
                "discount_type":event1.discount_type,
                "discount_value":event1.discount_value,
                "service_charge":sessions.first().eve_poc_id.sub_srv_id.cost
                # "singel_session_charge":session_charge,
                # "day_convinance":charge,
                # "session_with_convinance":session_with_convinance
            }
...
def create_payment_url(request):
    ...
    # order_id = "SPERO_W_ID" + timezone.now().strftime("%d%m%Y%H%M%S")
    order_id_w = "order_id_SPERO" + timezone.now().strftime("%d%m%Y%H%M%S")
        ...
        "messages": [
            {
                "from": from_number,
                "to": to_number,
                "messageId": order_id_w,
                "content": {
                    "templateName": template_name,
                    "templateData": {
                        "body": {
                            "placeholders": [amount, payment_link]
                        },
                    },
                    "language": "en"
                }
...
class agg_hhc_busy_days_of_profs_api(APIView):
    ...
    def post(self, request, format=None):
        ...
        end_time_added_15 =  datetime_obj_e + timedelta(minutes=14)

        datetime_obj_s = datetime.combine(datetime.today(), eve_poc_obj.start_time)
        start_time_added_15 =  datetime_obj_s - timedelta(minutes=14)
...
class create_module_Views(APIView):
    def post(self, request, *args, **kwargs):
        try :
            ...
            grp_name = data.get('grp_name', None)
            if not grp_name:
                return Response({"error": "grp_name is required."}, status=status.HTTP_400_BAD_REQUEST)
            if agg_mas_group.objects.filter(grp_name=grp_name).exists():
                return Response({"error": "This grp_name is already exists"}, status=status.HTTP_400_BAD_REQUEST)
            data['grp_code'] = f'UG-{grp_name}'
            ...
            return Response({"error": "Parse Error", "details": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class create_permission_module_Views (APIView):
    ...
    def post(self, request, *args, **kwargs):
        try :
            ...
            if Permission_module.objects.filter(name=name, group=group).exists():
                return Response({"error": "This Group and Module already exists."}, status=status.HTTP_400_BAD_REQUEST)

            serializer = New_module_serializer(data=data)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_201_CREATED)
            else:
    ...
    def post(self, request, *args, **kwargs):
        try :
            ...
            if permission.objects.filter(name=name, module=module).exists():
                return Response({"error": "This this Permission already exists."}, status=status.HTTP_400_BAD_REQUEST)

            serializer = add_New_permission_serializer(data=data)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_201_CREATED)
            else:
            ...
            return Response({"error1": "Validation Error", "details": str(e)}, status=status.HTTP_400_BAD_REQUEST)

        except ParseError as e:
            return Response({"error2": "Parse Error", "details": str(e)}, status=status.HTTP_400_BAD_REQUEST)

        except ObjectDoesNotExist as e:
            return Response({"error3": "Object not found", "details": str(e)}, status=status.HTTP_404_NOT_FOUND)

        except DatabaseError as e:
            return Response({"error4": "Database Error", "details": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class get_sub_moduel_Views(APIView):
    def get(self, request, group_id):
        try:
            if group_id:
                module = Permission_module.objects.filter(group=group_id)
                serializer = get_permission_serializer(module, many=True)
                return Response(serializer.data, status=status.HTTP_200_OK)
            else : 
                return Response({'error': 'Group Id not Provided'}, status=status.HTTP_404_NOT_FOUND)
...
class create_group_module_Views(APIView):
    # def get(self, request, *args, **kwargs):
    #     # Fetch all data from the Permission_module model
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Validate and save the group data in agg_mas_group
        group_serializer_instance = group_serializer(data=group_data)
        if group_serializer_instance.is_valid():
            group = group_serializer_instance.save()  # Save the group and get the instance
        else:
            return Response(group_serializer_instance.errors, status=status.HTTP_400_BAD_REQUEST)

        # Prepare module data, referencing the saved group instance
        module_data = request.data.copy()
        module_data['group'] = group.grp_id  # Use the primary key of the saved group

        # Validate and save the module data
...
class create_permission_Views(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if serializer.is_valid():
                # Save the valid data to the Permission_module model
                serializer.save()
                # Return a success response with the created data
                return Response(serializer.data, status=status.HTTP_201_CREATED)
            else:
                # If the data is invalid, raise a validation error
                raise ValidationError(serializer.errors)
        except ValidationError as ve:
            # Handle validation errors
            return Response({"error": "Validation Error", "details": ve.detail}, status=status.HTTP_400_BAD_REQUEST)
        except IntegrityError as ie:
            # Handle integrity errors, such as unique constraints or foreign key constraints
...
class SessionCancellationView(APIView):
    ...
    def post(self, request):
        ...
        if disc_type == 1:
            ...
            epppoc = agg_hhc_event_plan_of_care.objects.get(eve_id=eve_id, status = 1)
            allsess = len(epppoc.serivce_dates)
            sess_cnt_dtl = int(allsess) - int(record.count())
            sin_ses_disc = (epppoc.initail_discount_value/int(allsess)) if epppoc.initail_discount_value and allsess else 0
            dsc = (int(get_total_wout_cancel_session.count())+int(record.count()))
            total_disc_amt =(int(get_total_wout_cancel_session.count()) * sin_ses_disc)
            discounted_amt_wout_cncel = round(total_disc_amt)
        ...
        if event_cost_data.discount_type==2:
            event_cost_data.discount_value = discounted_amt_wout_cncel
...
class multiple_allocate_api(APIView):
    ...
    def post(self,request):
        ...
        if flag_id == 1:
            ...
            srv_prof_date_and_id = request.data.get('srv_prof_date_and_id',{})
            remark =  request.data.get('remark')
            print(srv_prof_date_and_id)
            
            for srv_prof_id, date_range_list in srv_prof_date_and_id.items():
                print(srv_prof_id,'srv_prof_id - keys')
                print(date_range_list,'date_range_list -- values')
                [[['2025-02-08', '2025-02-10']], 0, None]
                vv=date_range_list[0]
                print(vv)
                all_dates = []
                for date_list in vv:
                    if not date_list:
                ...
                amb=ambulance.objects.filter(ambs_id=date_range_list[2]).first()
    ...
    def get(self, request, eve_id):
        ...
        try:
            ...
            for i in events:
                ...
                total_conv_charge_is = sum(o.convinance_charges or 0 for o in conv_data)

                Total_amount = int(i.Total_cost) if i.Total_cost is not None else 0
                Final_amount = int(i.final_amount) if i.final_amount is not None else 0
                discount_type = i.discount_type

                #_________________________________________________discount type and given discount start__________________
                if i.discount_type in [1, 2]:
                    discount_value = int(i.discount_value) if i.discount_value is not None else 0
                else:
                    discount_value = 0
                ...
                per_prof_amount = (int(detaile_event_plan_of_care.last().eve_poc_id.sub_srv_id.cost) if detaile_event_plan_of_care.exists() and detaile_event_plan_of_care.last() else 0)
                ...
                for j in detaile_event_plan_of_care:
                    ...
                    #   day conv count################################################
                    conv_char = j.convinance_charges or 0  # Default to 0 if None
                    #   day conv count################################################
                    ...
                    if j.srv_prof_id is not None:
                        if str(j.srv_prof_id.prof_fullname) in professional_list:
                            ...
                            for k in get_dtl_data:
                                ...
                                last_time.append(str(k.end_time))
                            ...
                            if len(professional_name_date) == 1:
                                ...
                                professiona_proper_info = {
                                    ...
                                    'sessions': 1,
                                    'amount': per_prof_amount,
                                    'prof_tot_amt': per_prof_amount_is,
                                    'conv_charges': 1 * conv_char
                                }
                                ...
                                for l in professional_name_date:
                                    ...
                                    if len_count == len(professional_name_date):
                                        print(j, 'lllllllllllllllllllllllllll')
                                        ...
                                        professiona_proper_info = {
                                            ...
                                            'amount': per_prof_amount,
                                            'prof_tot_amt': per_prof_amount_is2,
                                            'conv_charges': session_count * conv_char
                                        }
                                        professional.append(professiona_proper_info)
                                        ...
                                        professiona_proper_info = {
                                            ...
                                            'sessions': session_count,
                                            'amount': per_prof_amount,
                                            'prof_tot_amt': per_prof_amount_is3,
                                            'conv_charges': session_count * conv_char
                                        }
                                        professional.append(professiona_proper_info)
                ...
                if payments:
                    print("paid")
                    amount_paid = int(payments.amount_paid) if payments.amount_paid is not None else 0
                    amount_remaining = int(payments.amount_remaining) if payments.amount_remaining is not None else 0
                    payment_mode = payments.mode
                    payment_date = str(payments.added_date)
                    utr = str(payments.utr)
                else:
                    amount_paid = 0
                    amount_remaining = Final_amount
                    payment_mode = None
                    utr = None
                ...
                service = {
                    ...
                    'preffered_proffesional': preffered_proffesional,
...
#----------------------------all dates is appended to the list end -------------------------------------
                    ...
                    sessions+=1
                payments=agg_hhc_payment_details.objects.filter(eve_id=i.eve_id,status=1,overall_status="SUCCESS").last()
                is_any_session_cancelled = agg_hhc_detailed_event_plan_of_care.objects.filter(
                eve_id=i.eve_id, is_cancelled=1, status=2
            ).exists()

                if is_any_session_cancelled:
                    if payments and payments.overall_status == "SUCCESS":
                    # Override the amount_paid with final_amount if a session is cancelled
                        amount_paid = i.final_amount
                        amount_remaining = 0  # Since amount_paid is set to final_amount
                        payment_mode=payments.mode if payments else None
                        payment_date = str(payments.added_date) if payments else None
                    else:
                        # If payment is not successful, set amount_paid to 0
                        amount_paid = 0
                        amount_remaining = i.final_amount
                        payment_mode = payments.mode if payments else None
                        payment_date = str(payments.added_date) if payments else None
                ...
                if(amount_remaining<=0):
                    ...
                    payment_details={'Total_amount':Total_amount,
                                     ...
                                     'amount_remaining':amount_remaining,
                                     'professional_amount':professional,
                                     'discount_value':discount_value if discount_type not in (3,) else Total_amount,
                                     'discount_type':discount_type
                                     }
                    completed_payment_list.append(payment_details)
                else:
...
class CancelPaymentLinkView(APIView):
    def post(self, request, event_id):
        # Get the list of link_ids associated with the event_id
        order_ids = self.get_link_ids_from_event(event_id)
        if not order_ids:
            return Response({"error": "No payment links found for the event."}, status=status.HTTP_400_BAD_REQUEST)
        
        # Cancel each payment link
        canceled_links = []
        failed_links = []
        
        for order_id in order_ids:
            # Cashfree cancel payment link URL
            cancel_url = f"https://api.cashfree.com/pg/links/{order_id}/cancel"
            ...
            try:
                ...
                if response.status_code == 200:
                    canceled_links.append(order_id)
                else:
                    failed_links.append({
                        "order_id": order_id,
...
class srv_enq_cancellation_data(APIView):
    def get(self, request):
        cancel_type = int(request.GET.get('cancel_flag'))
        month = int(request.GET.get('month_flag'))
        res_dt = []  
        if month == 13:
            tm = int(datetime.today().month)
            currentt_date = timezone.now().date()
            lsmdate = currentt_date - relativedelta(months=6)
            if 'can_res_id' in request.GET:
                can_res_id = int(request.GET.get('can_res_id'))

                if 'prof_alloc' in request.GET and int(request.GET.get('prof_alloc'))==1:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(event_id__event_status__in=[2,3],follow_up="2", event_id__status = 2,canclation_reason=can_res_id,last_modified_date__range=(lsmdate,currentt_date)).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(event_status__in=[2,3], srv_cancelled=1, status = 2, last_modified_date__range=(lsmdate,currentt_date))
                elif 'prof_alloc' in request.GET and int(request.GET.get('prof_alloc'))==2:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(event_id__event_status=1,follow_up="2", event_id__status = 2,canclation_reason=can_res_id,last_modified_date__range=(lsmdate,currentt_date)).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(event_status=1, srv_cancelled=1, status = 2, last_modified_date__range=(lsmdate,currentt_date))
                else:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2", event_id__status = 2,canclation_reason=can_res_id,last_modified_date__range=(lsmdate,currentt_date)).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(srv_cancelled=1, status = 2, last_modified_date__range=(lsmdate,currentt_date))
            else:
                if 'prof_alloc' in request.GET and int(request.GET.get('prof_alloc'))==1:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2",event_id__event_status__in=[2,3], event_id__status = 2, last_modified_date__range=(lsmdate,currentt_date)).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(event_status__in=[2,3], srv_cancelled=1, status = 2,last_modified_date__range=(lsmdate,currentt_date))
                elif 'prof_alloc' in request.GET and int(request.GET.get('prof_alloc'))==2:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2",event_id__event_status=1, event_id__status = 2, last_modified_date__range=(lsmdate,currentt_date)).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(event_status=1, srv_cancelled=1, status = 2,last_modified_date__range=(lsmdate,currentt_date))
                else:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2", event_id__status = 2, last_modified_date__range=(lsmdate,currentt_date)).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(srv_cancelled=1, status = 2,last_modified_date__range=(lsmdate,currentt_date))
            ...
            if 'can_res_id' in request.GET:
                can_res_id = int(request.GET.get('can_res_id'))
                if 'prof_alloc' in request.GET and int(request.GET.get('prof_alloc'))==1:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2", event_id__event_status__in=[2,3], event_id__status = 2,canclation_reason=can_res_id).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(event_status__in=[2,3], srv_cancelled=1, status = 2)
                elif 'prof_alloc' in request.GET and int(request.GET.get('prof_alloc'))==2:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2", event_id__event_status=1, event_id__status = 2,canclation_reason=can_res_id).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(event_status=1, srv_cancelled=1, status = 2)
                else:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2", event_id__status = 2,canclation_reason=can_res_id).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(srv_cancelled=1, status = 2)
            else:
                if 'prof_alloc' in request.GET and int(request.GET.get('prof_alloc'))==1:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2", event_id__event_status__in=[2,3], event_id__status = 2).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(event_status__in=[2,3], srv_cancelled=1, status = 2)
                elif 'prof_alloc' in request.GET and int(request.GET.get('prof_alloc'))==2:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2", event_id__event_status=1, event_id__status = 2).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(event_status=1, srv_cancelled=1, status = 2)
                else:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2", event_id__status = 2).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(srv_cancelled=1, status = 2)
        else:
            ...
            if 'can_res_id' in request.GET:
                can_res_id = int(request.GET.get('can_res_id'))
                if 'prof_alloc' in request.GET and int(request.GET.get('prof_alloc'))==1:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2",event_id__event_status__in=[2,3], event_id__status = 2,canclation_reason=can_res_id, last_modified_date__month = month, last_modified_date__year = year).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(event_status__in=[2,3], srv_cancelled=1, status = 2, last_modified_date__month = month, last_modified_date__year = year)
                elif 'prof_alloc' in request.GET and int(request.GET.get('prof_alloc'))==2:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2",event_id__event_status=1, event_id__status = 2,canclation_reason=can_res_id, last_modified_date__month = month, last_modified_date__year = year).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(event_status=1, srv_cancelled=1, status = 2, last_modified_date__month = month, last_modified_date__year = year)
                else:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2", event_id__status = 2,canclation_reason=can_res_id, last_modified_date__month = month, last_modified_date__year = year).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(srv_cancelled=1, status = 2, last_modified_date__month = month, last_modified_date__year = year)
            else:
                if 'prof_alloc' in request.GET and int(request.GET.get('prof_alloc'))==1:
                    print('in 1')
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2", event_id__event_status__in=[2,3], event_id__status = 2, last_modified_date__month = month, last_modified_date__year = year).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(event_status__in=[2,3], srv_cancelled=1, status = 2, last_modified_date__month = month, last_modified_date__year = year)
                elif 'prof_alloc' in request.GET and int(request.GET.get('prof_alloc'))==2:
                    print('in 2')
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2", event_id__event_status=1, event_id__status = 2, last_modified_date__month = month, last_modified_date__year = year).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(event_status=1, srv_cancelled=1, status = 2, last_modified_date__month = month, last_modified_date__year = year)
                else:
                    eq_fl = agg_hhc_enquiry_follow_up.objects.filter(follow_up="2", event_id__status = 2, last_modified_date__month = month, last_modified_date__year = year).order_by('event_id').distinct('event_id')
                    srv_dtl = agg_hhc_events.objects.filter(srv_cancelled=1, status = 2, last_modified_date__month = month, last_modified_date__year = year)

        if cancel_type == 1:     
            for i in eq_fl:
                if i.event_id:
                    if 'srv_flag' in request.GET:
                        srv = int(request.GET.get('srv_flag'))
                        gepoc = agg_hhc_event_plan_of_care.objects.filter(eve_id = i.event_id, srv_id = srv).last()
                    else:
                        gepoc = agg_hhc_event_plan_of_care.objects.filter(eve_id = i.event_id).last()
                    if gepoc is None:
                        continue
                    gdepoc = agg_hhc_detailed_event_plan_of_care.objects.filter(eve_id = i.event_id, status = 2, is_cancelled = 1)
                    if gdepoc:
                        ses_cnt = gdepoc.count()
                        srv_s_dt = gdepoc.aggregate(Min('actual_StartDate_Time'))['actual_StartDate_Time__min']
                        srv_e_dt = gdepoc.aggregate(Max('actual_StartDate_Time'))['actual_StartDate_Time__max']
                    else:
                        ses_cnt = 0
                        srv_s_dt = gepoc.start_date if gepoc else None
                        srv_e_dt = gepoc.end_date if gepoc else None
                    dt = {
                    "eve_id":i.event_id.eve_id,"event_code":i.event_id.event_code,"srv_start_date":srv_s_dt,"srv_end_date":srv_e_dt,"session_count":ses_cnt,"added_by":i.event_id.added_by,"last_modified_by":i.event_id.last_modified_by,"caller_id":i.event_id.caller_id.caller_id if i.event_id.caller_id else None,"caller_fullname":i.event_id.caller_id.caller_fullname if i.event_id.caller_id else None,"Patient_id":i.event_id.agg_sp_pt_id.agg_sp_pt_id if i.event_id.agg_sp_pt_id else None,"Patient_name":i.event_id.agg_sp_pt_id.name if i.event_id.agg_sp_pt_id else None,"pt_phone_no":i.event_id.agg_sp_pt_id.phone_no if i.event_id.agg_sp_pt_id else None,"pt_address":i.event_id.agg_sp_pt_id.address if i.event_id.agg_sp_pt_id else None,"pt_google_address":i.event_id.agg_sp_pt_id.google_address if i.event_id.agg_sp_pt_id else None,"pt_lattitude":i.event_id.agg_sp_pt_id.lattitude if i.event_id.agg_sp_pt_id else None,"pt_langitude":i.event_id.agg_sp_pt_id.langitude if i.event_id.agg_sp_pt_id else None,"pt_zone_id":i.event_id.agg_sp_pt_id.prof_zone_id.prof_zone_id if i.event_id.agg_sp_pt_id and i.event_id.agg_sp_pt_id.prof_zone_id else None,"pt_zone_name":i.event_id.agg_sp_pt_id.prof_zone_id.Name if i.event_id.agg_sp_pt_id and i.event_id.agg_sp_pt_id.prof_zone_id else None,"service_id":gepoc.srv_id.srv_id if gepoc and gepoc.srv_id else None,"service_title":gepoc.srv_id.service_title if gepoc and gepoc.srv_id else None,"sub_service_id":gepoc.sub_srv_id.sub_srv_id if gepoc and gepoc.sub_srv_id  else None,"sub_service_title":gepoc.sub_srv_id.recommomded_service if gepoc and gepoc.sub_srv_id else None,"cancel_type":"Enquiry Cancel","cancel_by": ("From_Spero" if i.cancel_by == 1 else "From_Patient" if i.cancel_by == 2 else "Other" if i.cancel_by else None),"cancel_reson_id":i.canclation_reason.cancelation_reason_id if i.canclation_reason else None,"cancel_reson":i.canclation_reason.cancelation_reason if i.canclation_reason else None,"cancellation_date":i.added_date,"remark": i.previous_follow_up_remark if i.previous_follow_up_remark else None
                    }
                    res_dt.append(dt)
        if cancel_type == 2:
            for i in srv_dtl:
                if i.event_id:
                    if 'srv_flag' in request.GET:
                        srv = int(request.GET.get('srv_flag'))
                        gepoc = agg_hhc_event_plan_of_care.objects.filter(eve_id = i.eve_id, srv_id = srv).last()
                    else:
                        gepoc = agg_hhc_event_plan_of_care.objects.filter(eve_id = i.eve_id).last()
                    if gepoc is None:
                        continue

                    if 'can_res_id' in request.GET:
                        can_res_id = int(request.GET.get('can_res_id'))
                        cnh = agg_hhc_cancellation_history.objects.filter(event_id = i.eve_id,reason=can_res_id).last()
                    else: 
                        cnh = agg_hhc_cancellation_history.objects.filter(event_id = i.eve_id).last()
                    if cnh is None:
                        continue
                    
                    

                    gdepoc = agg_hhc_detailed_event_plan_of_care.objects.filter(eve_id = i.eve_id, status = 2, is_cancelled = 1)
                    if gdepoc:
                        ses_cnt = gdepoc.count()
                        srv_s_dt = gdepoc.aggregate(Min('actual_StartDate_Time'))['actual_StartDate_Time__min']
                        srv_e_dt = gdepoc.aggregate(Max('actual_StartDate_Time'))['actual_StartDate_Time__max']
                        ...
                        srv_e_dt = gepoc.end_date if gepoc else None
                    dt = {
                    "eve_id":i.eve_id,"event_code":i.event_code,"srv_start_date":srv_s_dt,"srv_end_date":srv_e_dt,"session_count":ses_cnt,  "added_by":i.added_by,"last_modified_by":i.last_modified_by,"caller_id":i.caller_id.caller_id if i.caller_id else None,"caller_fullname":i.caller_id.caller_fullname if i.caller_id else None,"Patient_id":i.agg_sp_pt_id.agg_sp_pt_id if i.agg_sp_pt_id else None,"Patient_name":i.agg_sp_pt_id.name if i.agg_sp_pt_id else None,"pt_phone_no":i.agg_sp_pt_id.phone_no if i.agg_sp_pt_id else None,"pt_address":i.agg_sp_pt_id.address if i.agg_sp_pt_id else None,"pt_google_address":i.agg_sp_pt_id.google_address if i.agg_sp_pt_id else None,"pt_lattitude":i.agg_sp_pt_id.lattitude if i.agg_sp_pt_id else None,"pt_langitude":i.agg_sp_pt_id.langitude if i.agg_sp_pt_id else None,"pt_zone_id":i.agg_sp_pt_id.prof_zone_id.prof_zone_id if i.agg_sp_pt_id and i.agg_sp_pt_id.prof_zone_id else None,"pt_zone_name":i.agg_sp_pt_id.prof_zone_id.Name if i.agg_sp_pt_id and i.agg_sp_pt_id.prof_zone_id else None,"service_id":gepoc.srv_id.srv_id if gepoc and gepoc.srv_id else None,"service_title":gepoc.srv_id.service_title if gepoc and gepoc.srv_id else None,"sub_service_id":gepoc.sub_srv_id.sub_srv_id if gepoc and gepoc.sub_srv_id  else None,"sub_service_title":gepoc.sub_srv_id.recommomded_service if gepoc and gepoc.sub_srv_id else None,"cancel_type":"Service Cancel","cancel_by": ("From_Spero" if cnh.cancellation_by == 1 else "From_Patient" if cnh.cancellation_by == 2 else "From_Professional" if cnh.cancellation_by == 3 else None) if cnh.cancellation_by else None,"cancel_reson_id":cnh.reason.cancelation_reason_id if cnh.reason else None,"cancel_reson":cnh.reason.cancelation_reason if cnh.reason else None,"cancellation_date":i.last_modified_date,"remark": cnh.remark if cnh else None
                    }
                    res_dt.append(dt)
            ...
            for i in eq_fl:
                if i.event_id:
                    ...
                    if gdepoc:
                        ...
                        srv_e_dt = gepoc.end_date if gepoc else None
                    dt = {
                    "eve_id":i.event_id.eve_id,"event_code":i.event_id.event_code,"srv_start_date":srv_s_dt,"srv_end_date":srv_e_dt,"session_count":ses_cnt,"added_by":i.event_id.added_by,"last_modified_by":i.event_id.last_modified_by,"caller_id":i.event_id.caller_id.caller_id if i.event_id.caller_id else None,"caller_fullname":i.event_id.caller_id.caller_fullname if i.event_id.caller_id else None,"Patient_id":i.event_id.agg_sp_pt_id.agg_sp_pt_id if i.event_id.agg_sp_pt_id else None,"Patient_name":i.event_id.agg_sp_pt_id.name if i.event_id.agg_sp_pt_id else None,"pt_phone_no":i.event_id.agg_sp_pt_id.phone_no if i.event_id.agg_sp_pt_id else None,"pt_address":i.event_id.agg_sp_pt_id.address if i.event_id.agg_sp_pt_id else None,"pt_google_address":i.event_id.agg_sp_pt_id.google_address if i.event_id.agg_sp_pt_id else None,"pt_lattitude":i.event_id.agg_sp_pt_id.lattitude if i.event_id.agg_sp_pt_id else None,"pt_langitude":i.event_id.agg_sp_pt_id.langitude if i.event_id.agg_sp_pt_id else None,"pt_zone_id":i.event_id.agg_sp_pt_id.prof_zone_id.prof_zone_id if i.event_id.agg_sp_pt_id and i.event_id.agg_sp_pt_id.prof_zone_id else None,"pt_zone_name":i.event_id.agg_sp_pt_id.prof_zone_id.Name if i.event_id.agg_sp_pt_id and i.event_id.agg_sp_pt_id.prof_zone_id else None,"service_id":gepoc.srv_id.srv_id if gepoc and gepoc.srv_id else None,"service_title":gepoc.srv_id.service_title if gepoc and gepoc.srv_id else None,"sub_service_id":gepoc.sub_srv_id.sub_srv_id if gepoc and gepoc.sub_srv_id  else None,"sub_service_title":gepoc.sub_srv_id.recommomded_service if gepoc and gepoc.sub_srv_id else None,"cancel_type":"Enquiry Cancel","cancel_by": ("From_Spero" if i.cancel_by == 1 else "From_Patient" if i.cancel_by == 2 else "Other" if i.cancel_by else None),"cancel_reson_id":i.canclation_reason.cancelation_reason_id if i.canclation_reason else None,"cancel_reson":i.canclation_reason.cancelation_reason if i.canclation_reason else None,"cancellation_date":i.last_modified_date,"remark": i.previous_follow_up_remark if i.previous_follow_up_remark else None
                    }
                    res_dt.append(dt)
                    enq += 1
        ...
        if cancel_type == 1:
            return Response({"Total_Enquiry_Cancelled":len(res_dt),"Enquiry_Can_Data":res_dt})
        elif cancel_type == 2:
            return Response({"Total_Service_Cancelled":len(res_dt),"Service_Can_Data":res_dt})
        elif cancel_type == 3:
...
class update_sub_srv_eventwise(APIView):
    def post(self,request, eve_id, flag):
        try:
            pay = agg_hhc_payment_details.objects.filter(eve_id=eve_id, overall_status='SUCCESS', status = 1)
            if flag == 1 and not pay.exists():
                epl = agg_hhc_event_plan_of_care.objects.get(eve_id = eve_id, status = 1)
                sdu = request.data.get('is_srv_date_chng')
                s_t = datetime.strptime(request.data.get('start_time'), '%H:%M:%S').time()
                e_t = datetime.strptime(request.data.get('end_time'), '%H:%M:%S').time()
                if sdu == 1:
                    date_ranges = request.data.get('date_ranges', [])
                    def get_dates_between(start_date, end_date):
                        dates = []
                    ...
                    if gdpc:
                        ll = gdpc.last()
                        if epl.service_reschedule == 1:
                            epl.start_time = s_t
                            epl.end_time = e_t
                            epl.last_modified_by = 'abc'
                            epl.save()
                            print('epoc updated only')
                        else:
                            epl.No_session_dates = len(all_dates)
                            epl.serivce_dates = all_dates
                            epl.start_date = datetime.strptime(all_dates[0], '%Y-%m-%d').date()
                            epl.end_date = datetime.strptime(all_dates[-1],'%Y-%m-%d').date()
                            epl.last_modified_by = 'abc'
                            epl.save()

                            for index, i in enumerate(all_dates, start = 1):
                                for j in gdpc:
                                    j.status = 2
                                    j.save()
                                print(eve_id)
                                agg_hhc_detailed_event_plan_of_care.objects.create(
                                    eve_poc_id = epl if epl else None,
                                    eve_id = epl.eve_id if epl.eve_id else None,
                                    index_of_Session = index,
                                    actual_StartDate_Time = i,
                                    actual_EndDate_Time = i,
                                    start_time = s_t,
                                    end_time = e_t,
                                    status = 1,
                                    added_by = 'abc',
                            ...
                            print('ddddepoc updated only')
                epoc = agg_hhc_event_plan_of_care.objects.get(eve_id = eve_id,status = 1)
