...
class Dayprint(APIView):
    ...
    # permission_classes = [IsAuthenticated]
    def get(self, request):
        if int(request.query_params.get('hosp_id'))==0:
            start_date1=datetime.strptime(request.query_params.get('start_date'),'%Y-%m-%d').date()
            end_date1=(datetime.strptime(request.query_params.get('end_date'),'%Y-%m-%d')+timedelta(days=1)).date()
            hosp_id=request.query_params.get('hosp_id')
            mode=request.query_params.get('mode')
            
            if hosp_id:
                payment_entries=agg_hhc_payment_details.objects.filter( Q(payment_status=2) | Q(payment_status=3),overall_status='SUCCESS',payment_to_desk_date__range=(start_date1, end_date1),mode=mode, status=1)
            else:
                payment_entries=agg_hhc_payment_details.objects.filter(Q(payment_status=2)| Q(payment_status=3),overall_status='SUCCESS',payment_to_desk_date__range=(start_date1,end_date1),mode=mode,status=1)
            ...
            if hosp_id:
                payment_entries=agg_hhc_payment_details.objects.filter( Q(payment_status=2) | Q(payment_status=3),overall_status='SUCCESS',payment_to_desk_date__range=(start_date1, end_date1), eve_id__agg_sp_pt_id__preferred_hosp_id=hosp_id,mode=mode,status=1)
            else:
                payment_entries=agg_hhc_payment_details.objects.filter(Q(payment_status=2)| Q(payment_status=3),overall_status='SUCCESS',payment_to_desk_date__range=(start_date1,end_date1),mode=mode,status=1)
...
#         day_print = DayPrintSerializer(payment_entries, many=True)# Prepare CSV data
...
class DayPrint_excel(APIView):
    def get(self, request, start_date, end_date, hosp_id):
        ...
        if hosp_id != 0 and start_date == end_dates:
            ...
            payment_entries = agg_hhc_payment_details.objects.filter(
                ...
                payment_to_desk_date=start_date,
                status=1,
            )
        elif hosp_id != 0:
            epoc = agg_hhc_event_plan_of_care.objects.filter(hosp_id=hosp_id, status=1)
        ...
        # Prepare CSV data
        csv_data = self.serialize_to_csv(day_print.data, payment_entries)
    ...
    def serialize_to_csv(self, data, queryset):
        header = [
            "pay_dt_id",
            "eve_id",
            "event_code",
        ...
        # Aggregate amount_paid by mode
        mode_wise_totals = (
            queryset.values("mode")
            .annotate(total_amount=Sum("amount_paid"))
            .order_by("mode")
        )

        # **Generate dynamic mode mapping**
        unique_modes = list(mode_wise_totals.values_list("mode", flat=True))
        mode_map = {mode: idx + 1 for idx, mode in enumerate(unique_modes)}
        ...
        yield "\nSr.No,Mode of Payment,Amount Received\n"

        total_amount = 0
        for mode, sr_no in mode_map.items():
            amount = next(
                (entry["total_amount"] for entry in mode_wise_totals if entry["mode"] == mode), 0
            )
            total_amount += amount
            yield f"{sr_no},{mode},{amount}\n"

        # Yield total row
        yield f",Total,{total_amount}\n"

    def generate_csv_row(self, header, data_row):
        row = [str(data_row.get(field, "")) for field in header]
        return ",".join(row)
...
class Manage_emp(APIView):
    ...
    #         print("+==== ", user)
    #         if user:
    #             serializer = Manage_emp_serializer(user, data=rdata)
    #             if serializer.is_valid():
    #                 serializer.save()
    #                 res_data = {'msg':'User updated successfully.'}
    #                 response_data={
    #                 'Res_Data': res_data
    #                 }
...
#                     zone = agg_hhc_service_professionals.objects.filter(prof_registered=True, prof_interviewed=True, prof_doc_verified=True).order_by('prof_fullname')
#                 if zone:
#                     serializer = agg_hhc_srv_prof_serializer(zone, many=True)
#                     return Response(serializer.data, status=status.HTTP_200_OK)
#                 return Response({'msg':'No Data Found'}, status=status.HTTP_400_BAD_REQUEST)
#             elif sl == '1':
#                 Selected_prof_int = agg_hhc_prof_interview_details.objects.filter(int_status=1)
...
#             # else:
#             #     if pro:
#             #         zone = agg_hhc_service_professionals.objects.get(srv_prof_id=pro)
#             #     else:
#             #         # zone = agg_hhc_service_professionals.objects.filter(~Q(prof_registered=False, prof_interviewed=True, prof_doc_verified=True))
...
class interviews_schedule_prof_status_Views(APIView):
    ...
    def put(self, request, srv_pro_id):
        ...
        if serializer.is_valid():
            ...
            updated_data = {'interview_data': serializer.data}  # Collect interview data

            # If interview status is 1 (Selected)
            if serializer.validated_data.get('int_status') == 1:
                try:
                    ...
                    # Add updated professional data to response
                    updated_data['professional_data'] = {
            ...
            # If interview status is 2 (Rejected/Not Qualified)
                ...
                if not_qualified_serializer.is_valid():
                    ...
                    # Add not qualified data to response
...
class Prof_int_dtls_create_new_R(APIView):
    # def post(self, request):
    #     try:
    #         # Extract the data from the request
    #         rdata = request.data
    #         # Create a new instance of the serializer with the provided data
    #         serializer = agg_hhc_srv_prof_int_dtls_serializer(data=rdata)
    #         # Validate the data
    #         if serializer.is_valid():
    #             # Save the new record
    #             new_record = serializer.save()
    #             # Serialize the newly created record
    #             response_serializer = agg_hhc_srv_prof_int_dtls_serializer(new_record)
    #             res_data = {
    #                 'msg': 'Professional interview details created successfully.',
    #                 'data': response_serializer.data
    #             }
    #             return Response({'Res_Data': res_data}, status=status.HTTP_201_CREATED)
    #         # Return validation errors if data is not valid
    #         return Response({'Res_Data': {'msg': 'Invalid data', 'errors': serializer.errors}}, status=status.HTTP_400_BAD_REQUEST)
    #     except Exception as e:
    #         # Handle unexpected errors
    #         return Response({'Res_Data': {'msg': f'An error occurred: {str(e)}'}}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def post(self, request):
        try:
            ...
            rdata = request.data
            if not rdata:
                raise ParseError("Request data is empty.")
            ...
            return Response({'Res_Data': {'msg': 'Validation error occurred', 'error': str(val_error)}}, status=status.HTTP_400_BAD_REQUEST)
...
#             if query_params:
#                 employees = agg_hhc_service_professionals.objects.filter(prof_registered= True, prof_interviewed = True, prof_doc_verified =  True, **query_params.dict())
#             else:
#                 employees = agg_hhc_service_professionals.objects.filter(prof_registered= True, prof_interviewed = True, prof_doc_verified =  True)
...
class Edit_file_API_VIew(APIView):
    ...
    #             if not doc_li_id:
    #                 raise ValidationError("doc_li_id is required for each item.")

    #             # Check if the record with the same srv_prof_id and doc_li_id exists
    ...
    #             else:
    #                 # Ensure srv_prof_id is added to the new record data
...
class get_Prof_Payment_Detail_API(APIView):
    def get(self, request, srv_prof_id=None):
        try:
            ...
            datttttaaa=[]
            sud =  agg_hhc_professional_sub_services.objects.filter(srv_prof_id=payment_detail.srv_prof_id, Entry=2, status=1)
            for i in sud:
                dt = {
    ...
    def post(self, request, srv_prof_id=None):
        ...
        for record in request.data:
            ...
            Sub_srv_id = record.get('Sub_srv_id')
            record_srv_prof_id = record.get('srv_prof_id', srv_prof_id)
            from_date = record.get('form_date')
            to_date = record.get('to_date')
            cost = record.get('prof_cost')

            prof_id = agg_hhc_service_professionals.objects.get(srv_prof_id=record_srv_prof_id)
            # print(prof_sub_srv_id,"prof_sub_srv_id")
            sub_srv = agg_hhc_sub_services.objects.get(sub_srv_id = Sub_srv_id)
            print(sub_srv)
            instance = agg_hhc_professional_sub_services.objects.filter(srv_prof_id=record_srv_prof_id,sub_srv_id=Sub_srv_id, Entry=2).last()
            npser = agg_hhc_professional_sub_services.objects.create(
                srv_prof_id=prof_id,
                sub_srv_id=sub_srv,
                start_date = datetime.strptime(from_date, '%Y-%m-%d'),
                end_date = datetime.strptime("0001-01-01", '%Y-%m-%d'),
                prof_cost = cost,
                Entry = 2
                )
            # for i in instance:
            #     i.end_date =  datetime.strptime(from_date, '%Y-%m-%d') - timedelta(days=1)
            #     i.Entry = 1
            #     i.save()
        
            instance.end_date =  datetime.strptime(from_date, '%Y-%m-%d') - timedelta(days=1)
            instance.Entry = 1
            instance.save()
        ...
        return Response({"msg": "Done"}, status=status.HTTP_200_OK)
            ...
            # serializer = Prof_payement_details_serializer(instance, data=record, partial=True)
...
# ___________________________ External Professional Approved/Rejected _____________- Amit
...
# ______________________________________________________ Amit End _________________________________________

#  GET External Profesional 
class Exter_Prof_Action_Get_Views(APIView):

    def get(self, request):
        try:
            Prof_int = agg_hhc_service_professionals.objects.all().exclude(prof_compny=1, status=1)
            # Prof_int = agg_hhc_service_professionals.objects.all()
            serializer = Exter_Prof_Action_Get_Serializer(Prof_int, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
            ...
            return Response(
                {"error": error_message, "details": str(e), "traceback": traceback.format_exc()},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class Prof_id_card(APIView): #work pending
    # renderer_classes = [UserRenderer]
    # permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        try:
            pro = request.GET.get('pro')
            print('pro', pro)
            Prof_int = agg_hhc_service_professionals.objects.get(srv_prof_id = pro)
            print("prof dtl--", Prof_int)
            serializer = agg_hhc_srv_prof_serializer2(Prof_int, many=False)
    ...
    def put(self, request):
        ...
        try:
            ...
            if prof_int:
                serializer = agg_hhc_srv_prof_serializer2(prof_int, data=rdata)
                if serializer.is_valid():
                    serializer.save()
                    res_data = {'msg':'Professional details updated successfully.'}
                    response_data={
                    'Res_Data': res_data
                    }
                    return Response(response_data, status=status.HTTP_200_OK)
...
# ------------------------------- professional register API view HR -------------------------------
...
class edit(APIView):
    def get(self, request, srv_prof_id):
        try:
            ...
            if not service_professional_instance:
                return Response(
                    {"error": f"Service professional with ID {srv_prof_id} not found."},
                    status=status.HTTP_404_NOT_FOUND,
                )

            # Serialize service professional data
            service_professional_serializer = Add_Proffesional_Service_Professionals_Table_amit_Serializer(service_professional_instance)
            service_professional_data = service_professional_serializer.data

            # Fetch associated colleague details
            colleague_instance = agg_com_colleague.objects.filter(clg_ref_id=service_professional_instance.clg_ref_id).first()
            colleague_data = {}
            if colleague_instance:
                colleague_serializer = Add_Proffesional_Clg_Table_amit_Serializer(colleague_instance)
                colleague_data = colleague_serializer.data

            # Fetch associated CV details
            cv_instance = agg_hhc_service_professional_details.objects.filter(srv_prof_id=service_professional_instance.pk).first()
            cv_data = {}
            if cv_instance:
                cv_serializer = Professional_CV_Details_amit_Serializer(cv_instance)
                cv_data = cv_serializer.data
            ...
            if sub_services:
                sub_services_data = [
                    {
                        "prof_sub_srv_id": sub.prof_sub_srv_id,
                        "srv_prof_id": sub.srv_prof_id.srv_prof_id,
                        "sub_srv_id": sub.sub_srv_id.sub_srv_id,
                        "added_by": sub.added_by,
                        "last_modified_by": sub.last_modified_by,
                    }
            ...
            response_data = {
                "service_professional": service_professional_data,
    ...
    def put(self, request, srv_prof_id):
        ...
        try:
            ...
            ccsc = request.data.get('prof_zone')
            dddddd = ast.literal_eval(ccsc)
            priddd = agg_hhc_professional_zone.objects.get(prof_zone_id=dddddd[0])
            ...
            if zones:
                zone_list = ast.literal_eval(zones)
        ...
        #     # Check for phone number conflict and skip updating if there's a conflict
        #     if phone_no and phone_no != current_phone_no:
        #         # Check if the new phone number already exists for another service professional
        #         phone_conflict = agg_hhc_service_professionals.objects.filter(
        #             phone_no=phone_no
        #         ).exclude(srv_prof_id=srv_prof_id).exists()
        #         if phone_conflict:
        #             # If phone number conflict exists, skip updating phone_no, and inform the user
        #             phone_no = current_phone_no  # Keep the current phone_no

        #     # Check for email conflict and skip updating if there's a conflict
        #     if email_id and email_id != current_email_id:
        #         # Check if the new email ID already exists for another service professional
        #         email_conflict = agg_hhc_service_professionals.objects.filter(
        #             email_id=email_id
        #         ).exclude(srv_prof_id=srv_prof_id).exists()
        #         if email_conflict:
        #             # If email conflict exists, skip updating email_id, and inform the user
        #             email_id = current_email_id  # Keep the current email_id
        ...
        #     colleague_serializer = Add_Proffesional_Clg_Table_Serializer(colleague_instance, data=colleague_data, partial=True)
        #     if colleague_serializer.is_valid():
        #         colleague_serializer.save()
        #     else:
        #         return Response(colleague_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        ...
        #     service_professional_serializer = Add_Proffesional_Service_Professionals_Table_Serializer(
        #         service_professional_instance, data=service_professional_data, partial=True)
        #     if service_professional_serializer.is_valid():
        #         service_professional_serializer.save()
        #     else:
        ...
        try:
            ...
            with transaction.atomic():
                ...
                colleague_data = {
                    ...
                    'clg_mobile_no': data.get('contact_number'),
                    'clg_state': data.get('state'),
                    'clg_address': data.get('address'),
                    'added_by': data.get('added_by'),
                    'last_modified_by': data.get('last_modified_by'),
...
class event_wise_job_Closure_Revalidate_dtls(APIView):
    # renderer_classes = [UserRenderer]
    # permission_classes = [IsAuthenticated]
    def get(self, request, eve_id):
        dd = []
                    ...
                    if form_no_dt.exists():
                        if form_number == 2:
                            ...
                            if jcdddd and jc.exists():
                                ...
                                jcdddd = globals().get(f'mg_agg_hhc_session_job_closure_serializer_form_{form_number}', None)
                                print(f'jcdddd: {jcdddd}')  # Debugging line
                                if jcdddd and jc.exists():
...
class event_wise_job_clouser_dtls(APIView):
        ...
        try:
            ...
            for i in pc:
                if i.eve_id:
                    ...
                    form_no_dt = agg_hhc_jobclosure_form_numbering.objects.filter(prof_sub_srv_id=i.sub_srv_id.sub_srv_id, status = 1)
                    form_number = form_no_dt[0].form_number 
                    ddtl = agg_hhc_detailed_event_plan_of_care.objects.filter(eve_id=i.eve_id, status=1,  actual_StartDate_Time__lte=datetime.datetime.today(), job_closure_medical_gevournance=1)
                    for j in ddtl:
                        jc = agg_hhc_jobclosure_detail.objects.filter(dtl_eve_id=j.agg_sp_dt_eve_poc_id, status=1)
                        last_jc = jc.last()
                        if form_no_dt.exists():
    ...
    #                                 if not jcdddd:
    #                                     logger.error(f"Serializer for form number {form_number} is not defined.")
    #                                     keys_with_none = {field: None for field in ['is_job_closure_done', 'session_date']}
    #                                     keys_with_none.update({
    #                                         "srv_prof_id": j.srv_prof_id.srv_prof_id if j.srv_prof_id else None,
    #                                         "prof_name": {
    #                                             "prof_name": j.srv_prof_id.prof_fullname if j.srv_prof_id else None,
    ...
    #                                 # Proceed if the serializer exists
    #                                 # serializer_instance = jcdddd()
...
class edit_register_professional_from_hr(APIView):
    def get(self, request, srv_prof_id):
        try:
            ...
            # Fetch sub-services
            sub_services = agg_hhc_professional_sub_services.objects.filter(srv_prof_id=service_professional_instance, status = 1)
            sub_services_data = [
                {
            ...
            colleague_response = {
                ...
                # "clg_Date_of_birth": colleague_data.get("clg_Date_of_birth"),
                # "clg_joining_date": colleague_data.get("clg_joining_date"),
        ...
        try:
            ...
            def validate_file_size(file):
                ...
                if file.size > max_size:
                    return Response({"error": f"CV File size exceeds 5MB. Please provide a file less than 5MB."}, status=status.HTTP_409_CONFLICT)
                    # raise ValidationError("File size exceeds 5MB. Please provide a file smaller than 5MB.")
            ...
            if 'cv_file' in data:
                ...
                try:
                    ...
                    service_professional_data['prof_CV'] = cv_file
                except ValidationError as e:
            ...
            if sub_services:
                sub_services_list = ast.literal_eval(sub_services)
                existing_sub_services = agg_hhc_professional_sub_services.objects.filter(
                    srv_prof_id=service_professional_instance
                )

                # Step 1: Mark all existing entries as to-be-removed (status = 2)
                existing_ids = set(existing_sub_services.values_list('sub_srv_id__sub_srv_id', flat=True))
                request_ids = set(sub_services_list)

                for sub_service_id in existing_ids - request_ids:  # Entries in DB but not in request
                    entry_to_remove = existing_sub_services.get(sub_srv_id__sub_srv_id=sub_service_id)
                    if entry_to_remove.status != 2:
                        entry_to_remove.status = 2
                        entry_to_remove.save()

                # Step 2: Handle new and existing entries in the request
                for sub_service_id in sub_services_list:
                    sub_service = agg_hhc_sub_services.objects.get(sub_srv_id=sub_service_id)
                    existing_entry = existing_sub_services.filter(sub_srv_id=sub_service).first()

                    if existing_entry:
                        if existing_entry.status == 2:
                            # Step 4: Add a new entry for previously removed (status = 2) entries
                            psss = agg_hhc_professional_sub_services.objects.create(
                                srv_prof_id=service_professional_instance,
                                sub_srv_id=sub_service,
                                added_by=data.get('last_modified_by'),
                            ...
                            # Step 5: Entry exists and is active (status = 1), no changes needed
                            print(f"Entry already exists and is active for srv_prof_id: {service_professional_instance.srv_prof_id}, sub_srv_id: {sub_service.sub_srv_id}")
                    else:
...
#     # permission_classes = [IsAuthenticated]
    ...
    #         prof_id = agg_hhc_service_professionals.objects.get(srv_prof_id=prof_id, status=1)
    #         status_serializer = prof_status_serializer(prof_id)
    #         return Response(status_serializer.data)  # Corrected line
    #     except agg_hhc_service_professionals.DoesNotExist:
    #         return Response({"error": "Professional not found"}, status=404)
    ...
    #             return Response(status_serializer.data)
    #         else:
    #             return Response({"error": status_serializer.errors}, status=400)
    #     except agg_hhc_service_professionals.DoesNotExist:
    #         return Response({"error": "Professional not found"}, status=404)


class prof_documents_api(APIView):
    renderer_classes = [UserRenderer]
    permission_classes = [IsAuthenticated]
    def get(self, request):
        prof_id=request.query_params.get('prof_id')
        prof_doc=agg_hhc_documents_list.objects.filter(professional_role=agg_hhc_service_professionals.objects.get(srv_prof_id=prof_id, status=1).srv_id, status=1)
        doc_serializer = prof_doc_serializer(prof_doc, many=True)
        return Response(doc_serializer.data)
    
class submit_doc_api(APIView):
    def get(self, request):
        prof_id = request.query_params.get('prof_id')
        # prof=agg_hhc_service_professionals.objects.get(srv_prof_id = prof_id, status=1)
        prof_doc=agg_hhc_documents_list.objects.filter(professional_role=agg_hhc_service_professionals.objects.get(srv_prof_id=prof_id).srv_id)
        print(prof_doc[0].Documents_name, ';;;l;sadfd;;;;;')
        doc_list=documents_list_serializer(prof_doc, many=True)
        print(doc_list.data, ';adg')
        return Response(doc_list.data)
    
    def post(self, request):
        srv_prof_id = request.data.get('srv_prof_id')
        print(request.data.get('srv_prof_id'))
        doc_li_id = request.data.get('doc_li_id')
        doc = request.data.get('professional_document')
        data1={"srv_prof_id":srv_prof_id,"doc_li_id":doc_li_id,"professional_document":doc}
        print(data1, 'asdg')
        doc_serializer=post_documents_serializer(data=data1)
        if doc_serializer.is_valid():
            doc_serializer.save()
            return Response(doc_serializer.data)
        else: return Response({"error":doc_serializer.errors})
...
class Get_company_Details_API(APIView):
    ...
    def get(self, request, *args, **kwargs):
        companies = agg_hhc_company.objects.filter(status=1).order_by('-last_modified_date')
...
class Profesional_active_inactive_API(APIView):
    ...
    def get(self, request, srv_prof_id):
        snippet = agg_hhc_service_professionals.objects.get(srv_prof_id = srv_prof_id)
        serializers = Profesional_status_Serializer(snippet) 
        return Response(serializers.data,status=status.HTTP_200_OK)


    def put(self, request, srv_prof_id=None):
        try:
            serializer = Profesional_status_Serializer(data=request.data)
            if serializer.is_valid():
                serializer.save()
                pro = agg_hhc_service_professionals.objects.get(srv_prof_id = srv_prof_id)
                
                # Check the approve_reject field
                if request.data.get('status') == 1:  # Rejected
                    agg_hhc_prof_interview_details.objects.create(
                        srv_prof_id=pro,
                        # Schedule_Selected=2,
                        # hr_status=1,
                ...
                elif request.data.get('status') == 2:  # Approved
                    agg_hhc_prof_interview_details.objects.create(
                        srv_prof_id=pro,
                        # Schedule_Selected=1,
                        # hr_status=2,
...
class Company_Get_API(APIView):
    renderer_classes = [UserRenderer]
    permission_classes = [IsAuthenticated]
    def get(self,request):
        snippet = agg_hhc_company.objects.filter(status=1).order_by('company_name')
        serializers = Company_Serializer(snippet,many=True)
        return Response(serializers.data,status=status.HTTP_200_OK)
