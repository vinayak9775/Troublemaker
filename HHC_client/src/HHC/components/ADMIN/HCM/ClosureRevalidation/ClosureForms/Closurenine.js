...
const Closurenine = ({ getSessionDetils, formNo, evePlanID, profSD, profST, profED, profET, sessions, onClose, eveID }) => {
    const port = process.env.REACT_APP_API_KEY;
    const accessToken = localStorage.getItem('token');

    const [rtSize, setRTSize] = useState(null);
    ...
    console.log("Closure Form 9", startDate, startTime, endDate, endTime)
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setRTSize(sessionDetails[0]?.Size_RT || "");
    ...
    return (
        <>
            <Box sx={{ flexGrow: 1, bgcolor: "#ffffff", overflow: "auto", p: 3, maxHeight: "500vh", }}>
                ...
                <Grid item xs={12} sm={12} lg={12} sx={{ mt: 2, }}>
                    <Button variant="contained" sx={{ width: '25ch', borderRadius: "12px", ml: "4.6rem", textTransform: "capitalize" }}
                        onClick={handleClosureNineSubmit}
                    >Submit</Button>
                </Grid>
                <Snackbar
                    open={openSnackbar}
