  ...
  async function handleClosureTwoSubmit(event) {
    ...
    const questionDataArray = closureData.map((item, index) => {
      ...
      if (selectedValue === "1") {
        ...
        questionDataItem[`${item.jcq_id.date_time_remark_q_wise_name}`] =
    ...
    try {
      ...
      const response = await fetch(
        `${port}/web/job_closure_srv_sess_wise/${srvProfID}/${evePlanID}/`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
    ...
    <>
      ...
      >
        <div style={{ display: "flex" }}>
          ...
          <Button
            onClick={onClose}
            sx={{ marginLeft: "980px", marginTop: "2px", color: "gray" }}
          >
            <CloseIcon />
          </Button>
        </div>
            ...
            <>
              <Grid key={index} item xs={4} sm={4} md={4} lg={4}>
                ...
                >
                  <FormControl>
                    <FormLabel
                      id={`demo-row-radio-buttons-group-label-${index}`}
                    >
        ...
        <Grid item xs={12} sm={12} lg={12} sx={{ mt: 1, ml: "30rem" }}>
          <Button
            ...
            onClick={handleClosureTwoSubmit}
          >
            Submit
          </Button>
        </Grid>
